...
using ITLec.EmailTemplateManager.Controls;
using System;
...
{
    public partial class EmailTemplateEditorHelper : Form
    {
      ...
      //  private
        ...
        public string EmailTemplateXML { get; set; }

        public EmailTemplateEditorHelper()
        {
            InitializeComponent();
        }
        public EmailTemplateEditorHelper(string str)
        {
            this.strXmlLine = str;
            InitializeComponent();
        ...
        private void TvSiteMapNodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            ...
            var collec = (Dictionary<string, ITLec.CRMEmailTemplateGuy.Property>)selectedNode.Tag;
            ...
            BaseMainEmailTemplateUserControl  ctrl = null;

            string fullNodeName = TreeNodeHelper.FullNodeName(selectedNode);

            if (collec != null)
            {
                ctrl = new ITLec.EmailTemplateManager.Controls.MainEmailTemplateSectionControl(fullNodeName, collec);
                if (ctrl != null)
                {
        ...
        {
            if (!((IEmailTemplateSavable)panelContainer.Controls[0]).Save())
            {
                return;
            }
        ...
        private void toolStripButtonSaveSiteMapToDisk_Click(object sender, EventArgs e)
        {
            ...
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                ...
                var doc = new XmlDocument();
                XmlNode rootNode = doc.CreateElement("emailTemplates");
                doc.AppendChild(rootNode);
                ...
                MessageBox.Show(doc.SelectSingleNode("emailTemplate").OuterXml);
                siteMapDoc.LoadXml(doc.SelectSingleNode("emailTemplate").OuterXml);
        ...
        private void AddXmlNode(TreeNode currentNode, XmlNode parentXmlNode, bool hasDisabledParent = false)
        {
            ...
            var collec = (Dictionary<string, ITLec.CRMEmailTemplateGuy.Property>)currentNode.Tag;
            if (collec != null)
            {
        ...
        private void AppendCustomProperties(XmlNode parentXmlNode, Dictionary<string, ITLec.CRMEmailTemplateGuy.Property> collec)
        {
