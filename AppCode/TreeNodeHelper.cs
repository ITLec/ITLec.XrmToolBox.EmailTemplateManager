...
namespace ITLec.CRMEmailTemplateGuy.AppCode
{
    ...
    internal class TreeNodeHelper
    {
        ...
        public static void AddContextMenu(TreeNode node, EmailTemplateEditorHelper form)
        {
            var collec = (Dictionary<string, ITLec.CRMEmailTemplateGuy.Property>)node.Tag;

            HideAllContextMenuItems(form.nodeMenu);
            string nodeFullName = TreeNodeHelper.FullNodeName(node);
            ///////////////////////////
           var currentSection = Common.EmailTemplateStructure.Sections.Where(e => e.Name == nodeFullName).FirstOrDefault();
            ...
            if (currentSection != null)
            {
                ...
                {
                    // node.
                    XmlNode emailTemplateEditorXmlNode = form.siteMapDoc.DocumentElement;
                    bool existedSubSection = true;
                    //Add Annotation
                    foreach (XmlNode _node in emailTemplateEditorXmlNode.ChildNodes)
                    {
        ...
        private static void subSectionMenuOnClick(object sender, EventArgs eventArgs, TreeNode node)
        {
            ...
            /*subNode.Tag*/ var section = Common.EmailTemplateStructure.Sections.Where(e => e.Name == item.Name).FirstOrDefault();

            if (section != null)
            {

                var properties = section.Properties;
                Dictionary<string, ITLec.CRMEmailTemplateGuy.Property> arr = new Dictionary<string, Property>();
        ...
        public static void AddTreeViewNode(object parentObject, XmlNode xmlNode, EmailTemplateEditorHelper form, bool isDisabled = false)
        {
            TreeNode node = new TreeNode(xmlNode.Name);

            Dictionary<string, ITLec.CRMEmailTemplateGuy.Property> attributes = new Dictionary<string,  ITLec.CRMEmailTemplateGuy.Property>();
            ...
            if (xmlNode.NodeType != XmlNodeType.Text)
            {
                foreach (XmlAttribute attr in xmlNode.Attributes)
                {
                    if (attr.Name == "CustomProperties")
                    {
                        ...
                        Dictionary <string, ITLec.CRMEmailTemplateGuy.Property> customAttributes = new Dictionary<string, ITLec.CRMEmailTemplateGuy.Property>();
                        ...
                        foreach (string propertyItem in propertyItems)
                        {
                            if (!string.IsNullOrEmpty(propertyItem))
                            {
                                ...
                                ITLec.CRMEmailTemplateGuy.Property pro = new Property() { Value = propertyItemValue, Name = propertyItemKey };
                    ...
                    {
                        ITLec.CRMEmailTemplateGuy.Property pro = new Property() {Value= attr.Value, Name = attr.Name};
                        attributes.Add(attr.Name, pro);
                    }
                    if(attr.Name == "EmailTemplateType")
                    {
                        EmailTemplateEditorHelper.EmailTemplateType = attr.Value;
                    }
                }
